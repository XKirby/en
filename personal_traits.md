...
{: .tip-callout }
> *The combination of Aquatic and Carrier can be a huge help to land-bound allies. Bloopers and Cheep Cheeps are great at this, giving you a chance to shine whenever the water calls to your adventure. You can even ferry people around during battle!* ![](assets/images/icons/tipguy.png)
> {: .icon-right }
...
> 
> - *Copies*{: .header-font }  
    Your split characters are perfect copies of yourself, requiring you to make a *Steady*{: .courage-color } check as an action to split into two or for any two clones to re-combine. With a *Great*{: .great-color } result this becomes a free action, but otherwise both clones lose their action for the turn.  
    When split apart, each copy is weaker than the sum of its parts; both the original character and the clone each have their max *HP*{: .heart-color }, max *FP*{: .spirit-color }, *BP*{: .courage-color }, Stats and Skills effectively halved.  
    You all retain your existing *HP*{: .heart-color } and *FP*{: .spirit-color }, up to the limit of your new maximums, and all clones share access to the same *SP*{: .courage-color } pool, as long as they're under your control.  
    When re-combining, any *HP*{: .heart-color } or *FP*{: .spirit-color } loss is carried over from the clones, and your overall abilities are recalculated as appropriate; you never risk losing stats to a rounding error.
>
>    {: .example-callout}
>    > *Example: Fuzzy Splits*{: .header-font }
>    >
>    > A Fuzzy with *15 HP*{: .heart-color } splits into two Fuzzies, who each have *7 HP*{: .heart-color }. One of the clones then takes 1 damage, going down to *6/7 HP*{: .heart-color }.  
>    > When re-combining, the Fuzzy goes back to having *15 HP*{: .heart-color } at maximum.  
>    > Since one of the clones took 1 damage, the re-combined Fuzzy has *14/15 HP*{: .heart-color }.
>
>    Each clone is able to split into another pair of clones as an action, though you can only split into more clones if you have enough *HP*{: .heart-color } to split.  
>    As long as you retain control of the clones, they each have the combined knowledge and awareness of every other clone.  
>    It's difficult to divide your consciousness up like this; for every 10 turns that passes with your clones split apart, you must make a *Steady*{: .courage-color } check using your original stats and skills, requiring a Grade of Success for every 10 turns that's already passed.  
>    If you fail, you lose control of all active clones and are completely Stunned (6) for a turn.  
>    Resting can restore your lost abilities after losing control, but all your clones will remain active – and most likely, hostile.
>
> - *Illusions*{: .header-font }  
...
> This generally requires an *Athletics*{: .heart-color } check, and the flying character is often restricted by the height they can gain and the distance they can fly before resting.  
> Only *Aerial*{: .positive-color }, *Reach*{: .positive-color } or *Ranged*{: .positive-color } attacks can strike a flying target.  
> Flying characters can make melee attacks against targets anywhere in battle.  
> These function similarly to *Aerial*{: .positive-color } attacks, although Plummeting and triggering Weakness (Aerial) still requires the *Aerial*{: .positive-color } trait.  
> A flying character struck by an *Aerial*{: .positive-color }, *Launch*{: .positive-color } or otherwise powerful attack must make a *Steady*{: .courage-color } check or be knocked to the ground.
...
> Despite their hovering above the ground, melee attacks can still reach them just fine, though they're safe from *Quake*{: .positive-color } or *Underfoot*{: .positive-color } attacks.
...
> Typically this effect costs *5 FP*{: .spirit-color }, though the exact cost and duration depends on the source of this power.
...
> A character with this ability can make a *Magic*{: .spirit-color } or *Trickery*{: .spirit-color } check as an action to transform completely into another creature. This transformation is a complete physical transformation with an effect on the user's psyche, giving them all the Traits, Techniques and other physical qualities of their new form – even their Skill Specialisations!  
...
> Transformations become harder and more expensive to maintain the less familiar you are with the intended form. For each Grade of Success achieved beyond the requirements, you can pay *1 less FP*{: .spirit-color } per turn – potentially negating the FP cost altogether.
> 
...
> ## Resist
> 
> A character with this Trait has some measure of Defense versus certain effects, even protecting them from Piercing attacks; however this Defense bonus *only*{: .underlined } applies to the specified kind of attack.  
